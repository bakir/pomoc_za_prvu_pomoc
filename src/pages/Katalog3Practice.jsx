...
export default function Katalog3Practice() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await fetch('./katalog3/k3_questions.json');
        ...
        console.error('Failed to load katalog3 questions:', error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const updateSettings = useCallback((patch) => {
    setSettings((prev) => {
      ...
      localStorage.setItem(KATALOG3_SETTINGS_KEY, JSON.stringify(next));
      return next;
    });
  }, []);
  ...
  const handleReview = (value) => {
    saveKatalog3Review(currentQuestionId, value);
    setQuestionReview(value);
  };

  const handleResetProgress = () => {
    if (window.confirm('Resetovati napredak za raskrsnice?')) {
      clearKatalog3Progress();
      setProgress({});
      resetQuestionState();
      ...
      if (visibleCount > 0 && masteredQuestions === visibleCount) {
        return (
          <div className="card completion-screen">
            <h1>🎉 Čestitamo! 🎉</h1>
            <p>Savladali ste sva pitanja o raskrsnicama!</p>
          </div>
        );
      }
      return (
      ...
      <div className="card">
        ...
        {question.image && (
          <div className="question-image-wrap">
            <img
              src={katalog3AssetUrl(question.image)}
              alt={`Saobraćajna situacija za pitanje ${currentQuestionId}`}
              ...
              {question.question_pic && (
                <div className="question-pic-review">
                  ...
                  <div className="question-pic-wrap">
                    <img
                      src={katalog3AssetUrl(question.question_pic)}
